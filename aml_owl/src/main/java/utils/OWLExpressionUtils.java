...
import translation.ontology.AttributeHandler;
...
//		return isConjunctive(ce) || isDisjunctive(ce) || isRestriction(ce);
	...
	public static String getAMLClassName (OWLClassExpression ce) {
		String iri = getAMLClass(ce).getIRI().toString();
		
		if(!iri.contains(Consts.aml_pref))
			return getAMLClass(ce).getIRI().getShortForm();
		
		return iri.substring(Consts.aml_pref.length());						
	}		
...
//		String name = individual.getIndividualsInSignature().iterator().next().getIRI().getShortForm();
	...
	public static boolean hasAMLInverseRoleAsProperty(OWLObjectPropertyExpression ope) {
		if(AMLObjectPropertyIRIs.IS_IE_OF.toString().equals((getAMLPropertyName(ope))) ||
				AMLObjectPropertyIRIs.IS_EI_OF.toString().equals((getAMLPropertyName(ope))) ||
				AMLObjectPropertyIRIs.IS_IE_OF_TC.toString().equals((getAMLPropertyName(ope))) ||
				AMLObjectPropertyIRIs.IS_EI_OF_TC.toString().equals((getAMLPropertyName(ope)))){
			return true;
		}
		return false;
	}
