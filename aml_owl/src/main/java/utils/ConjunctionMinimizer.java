...
public class ConjunctionMinimizer implements OWLClassExpressionVisitorEx<OWLClassExpression>{
	...
	public OWLClassExpression visit(OWLObjectSomeValuesFrom ce) {
		...
		if(AMLObjectPropertyIRIs.IS_IE_OF.toString().contains(OWLExpressionUtils.getAMLPropertyName(ce.getProperty()))) {
			if(newFiller instanceof OWLObjectIntersectionOf) {
				for(OWLClassExpression op : ((OWLObjectIntersectionOf) newFiller).getOperands()) {
					if(op instanceof OWLObjectSomeValuesFrom) {
						...
						if(!AMLObjectPropertyIRIs.HAS_IE.toString().contains(opRoleName) && ((OWLObjectSomeValuesFrom) op).getFiller().equals(owlFactory.getOWLThing()))
							validFillers.add(op);
					}else
						validFillers.add(op);
				}
