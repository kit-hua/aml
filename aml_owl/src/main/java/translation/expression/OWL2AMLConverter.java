...
public class OWL2AMLConverter implements OWLClassExpressionVisitorEx<GenericAMLConceptModel<AMLConceptConfig>>{
	...
	private CAEXObject getCAEXObjectSimple(OWLObjectPropertyExpression ope) {
		...
		else if(AMLObjectPropertyIRIs.HAS_IE_TC.toString().equals(propertyName))
		{
			return getCAEXObjectSimple(AMLClassIRIs.INTERNAL_ELEMENT);
		}
		else if(AMLObjectPropertyIRIs.HAS_EI_TC.toString().equals(propertyName))
		{
			return getCAEXObjectSimple(AMLClassIRIs.EXTERNAL_INTERFACE);
		}
...
//					return null;
	...
	public GenericAMLConceptModel<AMLConceptConfig> visit(OWLObjectSomeValuesFrom ce) {
		...
		if(caex != null) {			
			...
			String propertyName = OWLExpressionUtils.getAMLPropertyName(property);
			// if descendant axis, i.e. hasIE+, hasEI+, then set "descendent" to true.
			if(AMLObjectPropertyIRIs.HAS_IE_TC.toString().equals(propertyName))
			{
				config.setDescendant(true);
			}
			if(AMLObjectPropertyIRIs.HAS_EI_TC.toString().equals(propertyName))
			{
				config.setDescendant(true);
			}
