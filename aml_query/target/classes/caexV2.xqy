...
(:==============================================================================
: Part 3 - Data exchange helper functions
: - caex215:copy($element as element()) as element()
: - caex215:isProjectDefault ($node as node()) as xs:boolean
==============================================================================:)
...
declare function caex215:isIEconnectedViaEIType ($ie1 as node(), $ie2 as node(), $ic as xs:string) as xs:boolean {
  if(
    for $ei1 in $ie1/ExternalInterface[@RefBaseClassPath=$ic]
    for $ei2 in $ie2/ExternalInterface[@RefBaseClassPath=$ic]
      return (
        if (caex215:connectsTo($ei1, $ei2))
        then (true())
        else ()
      )
  )
...
declare function caex215:isEqualValue ($attribute1 as node(), $attribute2 as node()) as xs:boolean {
  if (
