	...
	private String getDataPropertyName (GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>> node) {
		...
		if(node.data.getObj() instanceof AttributeType) {
			if(node.getParent() != null)
				name = getDataPropertyName((GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>>) node.getParent()) + "_" + ((CAEXObject)node.data.getObj()).getName();
			else
				name = ((CAEXObject)node.data.getObj()).getName();
		}
...
//		return ce;
	...
	private OWLClassExpression toOWLClassExpressionRecursive (GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>> node, boolean isEntryPoint) {
		...
		OWLClassExpression ce = null;
		CAEXObject obj = (CAEXObject) node.data.getObj();
...
//			IRI iri = IRI.create(Consts.importer_pref + "has_" + attr.getName());
			IRI iri = IRI.create(Consts.aml_pref + "has" + getDataPropertyName(node));
...
//						ce = owlFactory.getOWLObjectIntersectionOf(ce, dhv);												
				...
				if(attr.getConstraint().get(0).getOrdinalScaledType() != null) {
					...
					if(ost.getRequiredMinValue() != null) {
						String minStr = AMLHelperFunctions.fromAMLAnyType(ost.getRequiredMinValue());
						if(minStr != null) {
							OWLLiteral min = owlFactory.getOWLLiteral(Double.valueOf(minStr));
					...
					if(ost.getRequiredMaxValue() != null) {
						String maxStr = AMLHelperFunctions.fromAMLAnyType(ost.getRequiredMaxValue());
						if(maxStr != null) {
							OWLLiteral max = owlFactory.getOWLLiteral(Double.valueOf(maxStr));
							OWLDataRange range = owlFactory.getOWLDatatypeRestriction(owlFactory.getDoubleOWLDatatype(), OWLFacet.MAX_INCLUSIVE, max);
...
//			}									
		...
		// External interfaces are mapped to object relations to its referenced class
			...
			if(config.isDescendant())
				property = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.HAS_EI_TC);						
			else
				property = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.HAS_EI);
			...
			if(ei.getRefBaseClassPath() != null) {
				...
				/**
				 * updating from simple class names to full paths
				 */
...
//				}
			...
			if(!isEntryPoint)
				ce = toOWLClassExpression(property, filler, config);
			else
				ce = filler;
		}// end EI
...
//				}
		...
		// Internal elements are mapped to object relations to its referenced class
			...
			OWLObjectProperty property;
			if(config.isDescendant())
				property = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.HAS_IE_TC);
			else
				property = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.HAS_IE);
...
//			}
			...
			for(SupportedRoleClassType src : ie.getSupportedRoleClass()) {
				String classref = src.getRefRoleClassPath();
				String className = classref;
				
				/**
				 * updating from simple class names to full paths
				 */				
				owlClasses.add(owlFactory.getOWLClass(IRI.create(className)));				
			}
			
			if(owlClasses.size() == 1) {
				filler = owlClasses.iterator().next();
				// check if we need to negate the local class reference
				// Notice that the negation shall not be applied to child attributes
				if(config.isNegated())
					filler = tryBuildComplement(filler, config);
			...
			if(owlClasses.size() > 1) {
				filler = owlFactory.getOWLObjectIntersectionOf(owlClasses);
				// if more than one SRC, we ignore the negation, since we do not know which SRC shall be negated
				if(config.isNegated())
					System.err.println("AML2OWLConvert.toOWLClassExpressionRecursive: negated ACM object with more than one referenced role classes!");
			}
	...
	private OWLClassExpression getParentOWLClassExpressionRec (GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>> node) {
		...
		if(config.isDescendant())
		{
			if(node.data.getObj() instanceof InternalElementType)
				inverseProperty = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.IS_IE_OF_TC);
			else if(node.data.getObj() instanceof ExternalInterfaceType)
				inverseProperty = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.IS_EI_OF_TC);
			else {
			...
			if(node.data.getObj() instanceof InternalElementType)
				inverseProperty = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.IS_IE_OF);
			else if(node.data.getObj() instanceof ExternalInterfaceType)
				inverseProperty = owlFactory.getOWLObjectProperty(AMLObjectPropertyIRIs.IS_EI_OF);
			else {
				System.err.println("AML2OWLConvert.getParentOWLClassExpressionRec: cannot generate inverse property for the node: " + node);
				return null;
			}	
