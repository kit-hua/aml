...
public static <T extends IAMLConceptConfig> CAEXObject toConfiguredCAEXObject (GenericTreeNode<GenericAMLConceptModel<T>> root) {	
		
		CAEXObject obj = root.data.toConfiguredCAEXObject();
		...
		if (obj instanceof AttributeType) {
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) {				 				
				if(child.data.getObj() instanceof AttributeType) 
						attr.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));								
			}
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) { 				
				if(child.data.getObj() instanceof AttributeType)
					ei.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));
			}
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) { 				
				if(child.data.getObj() instanceof AttributeType)
					ic.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));
			}
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) { 				
				if(child.data.getObj() instanceof AttributeType)
					rc.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));
				if(child.data.getObj() instanceof ExternalInterfaceType)
					rc.getExternalInterface().add((ExternalInterfaceType) toConfiguredCAEXObject(child));				
			}
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) { 				
				if(child.data.getObj() instanceof AttributeType)
					ie.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));
				if(child.data.getObj() instanceof ExternalInterfaceType)
					ie.getExternalInterface().add((ExternalInterfaceType) toConfiguredCAEXObject(child));
				if(child.data.getObj() instanceof InternalElementType)
					ie.getInternalElement().add((InternalElementType) toConfiguredCAEXObject(child));
			}
			...
			for(GenericTreeNode<GenericAMLConceptModel<T>> child : root.getChildren()) { 				
				if(child.data.getObj() instanceof AttributeType)
					suc.getAttribute().add((AttributeType) toConfiguredCAEXObject(child));
				if(child.data.getObj() instanceof ExternalInterfaceType)
					suc.getExternalInterface().add((ExternalInterfaceType) toConfiguredCAEXObject(child));
				if(child.data.getObj() instanceof InternalElementType)
					suc.getInternalElement().add((InternalElementType) toConfiguredCAEXObject(child));
			}
...
//		private static CAEXObject removeConfig (CAEXObject obj) {
	private static CAEXBasicObject removeConfig (CAEXBasicObject obj) {
		if(obj instanceof AttributeType) {
			AttributeType attr = (AttributeType) obj;			
...
//			CAEXObject cObj = child.data.getObj();
			CAEXBasicObject cObj = child.data.getObj();
...
//			CAEXObject cObjNoConig = removeConfig(cObj);
			CAEXBasicObject cObjNoConig = removeConfig(cObj);
...
//					CAEXObject oObjNoConfig = removeConfig(oObj);
					CAEXBasicObject oObj = other.data.getObj();
					CAEXBasicObject oObjNoConfig = removeConfig(oObj);
					...
					if(EcoreUtil.equals(cObjNoConig, oObjNoConfig)) {											
						...
						if(child.data.getConfig().canBeFusedWith(other.data.getConfig())) {
							if(child.data.getConfig() instanceof BasicAMLConceptConfig && other.data.getConfig() instanceof BasicAMLConceptConfig) {
								...
								if( ((BasicAMLConceptConfig) child.data.getConfig()).isDistinguished() || ((BasicAMLConceptConfig) other.data.getConfig()).isDistinguished() )
									((BasicAMLConceptConfig) child.data.getConfig()).setDistinguished(true);
							}							
						}												
	...
	public static <T extends IAMLConceptConfig> Set<GenericTreeNode<GenericAMLConceptModel<T>>> getPrimaryObjs (GenericTreeNode<GenericAMLConceptModel<T>> root) {
		...
		for(GenericTreeNode<GenericAMLConceptModel<T>> node : root.getDescendantOrSelf()) {
			if(node.data.getConfig() instanceof BasicAMLConceptConfig) {
				if(((BasicAMLConceptConfig) node.data.getConfig()).isDistinguished())
					primaries.add(node);
			}				
		}
	...
	private static <T extends IAMLConceptConfig> GenericTreeNode<GenericAMLConceptModel<T>> parse (AttributeType attr, Class<T> type) throws InstantiationException, IllegalAccessException {
		...
		if(!GenericAMLConceptModel.isConfigAttribute(attr)) {
			root.data.fromConfiguredCAEXObj(attr);
			for(AttributeType sub : attr.getAttribute()) {					
				if(!GenericAMLConceptModel.isConfigAttribute(sub)) {					
...
//					}
	...
	public static <T extends IAMLConceptConfig> GenericTreeNode<GenericAMLConceptModel<T>> parse (CAEXObject obj, Class<T> type) throws InstantiationException, IllegalAccessException{
		...
		if(obj instanceof InstanceHierarchyType) {
			InstanceHierarchyType ih = (InstanceHierarchyType) obj;
			root.data.fromCAEXObject(obj);			
...
//			}
	...
	public static GenericTreeNode<GenericAMLConceptModel<AMLTargetConfig>> parseTarget (CAEXBasicObject obj) throws InstantiationException, IllegalAccessException{
		
		GenericTreeNode<GenericAMLConceptModel<AMLTargetConfig>> root = new GenericTreeNode<GenericAMLConceptModel<AMLTargetConfig>>();
		root.data = new GenericAMLConceptModel<AMLTargetConfig>(AMLTargetConfig.class);		
		
		if(obj instanceof InstanceHierarchyType) {
			InstanceHierarchyType ih = (InstanceHierarchyType) obj;
			root.data.fromCAEXObject(ih);			
			
			for(InternalElementType ie : ih.getInternalElement()) {
				root.addChild(parseTarget(ie));
			}
		}
		...
		// Note: this will only be called if the root is not "queryConfig"!
			...
			AttributeType attr = (AttributeType) obj;
			if(!GenericAMLConceptModel.isConfigAttribute(attr)) {
				root.data.fromCAEXObject(attr);
				for(AttributeType sub : attr.getAttribute()) {					
					if(!GenericAMLConceptModel.isConfigAttribute(sub)) {					
						root.addChild(parseTarget(sub));
					}
				}	
...
//				}
			...
			ExternalInterfaceType ei = (ExternalInterfaceType) obj;
			root.data.fromCAEXObject(ei);
			
			for(AttributeType attr : ei.getAttribute()) {
				if(!GenericAMLConceptModel.isConfigAttribute(attr)) {
					root.addChild(parseTarget(attr));
				}
			}
...
//			}
			...
			for(SupportedRoleClassType src : ie.getSupportedRoleClass()) {
				root.addChild(parseTarget(src));
			}
			
			if(ie.getRoleRequirements() != null)
				root.addChild(parseTarget(ie.getRoleRequirements()));
			// clear the children of ie: IL and SRC are not CAEX Object children
//			if(type == AMLConceptConfig.class) {
//				ie.getAttribute().clear();
//				ie.getExternalInterface().clear();
...
//			}
		...
		else if(obj instanceof RoleRequirementsType) {
			RoleRequirementsType rr = (RoleRequirementsType) obj;
			root.data.setObj(rr);
			AMLTargetConfig config = new AMLTargetConfig();
			// a RR in target is always new: we cannot bind it to a source object
			config.setMode(AMLTargetConfig.ExchangeMode.NEW);
			root.data.setConfig(config);
		}
		
		else if(obj instanceof SupportedRoleClassType) {
			SupportedRoleClassType src = (SupportedRoleClassType) obj;
			root.data.setObj(src);
			AMLTargetConfig config = new AMLTargetConfig();
			// a SRC in target is always new: we cannot bind it to a source object
			config.setMode(AMLTargetConfig.ExchangeMode.NEW);
			root.data.setConfig(config);
		}
	...
	public static GenericTreeNode<GenericAMLConceptModel<AMLQueryConfig>> fromACMToAMLQueryModel (GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>> acmRoot) throws InstantiationException, IllegalAccessException{
		
		GenericTreeNode<GenericAMLConceptModel<AMLQueryConfig>> root = new GenericTreeNode<GenericAMLConceptModel<AMLQueryConfig>>();
		root.data = new GenericAMLConceptModel<AMLQueryConfig>(AMLQueryConfig.class);		
		
		AMLQueryConfig config = (AMLQueryConfig) root.data.getConfig();
		config.fromAMLConceptConfig((AMLConceptConfig) acmRoot.data.getConfig());
		
		root.data.setObj(acmRoot.data.getObj());
//		CAEXObject acmRootCaex = acmRoot.data.toConfiguredCAEXObject();
//		root.data.fromConfiguredCAEXObj(acmRootCaex);
		
		for(GenericTreeNode<GenericAMLConceptModel<AMLConceptConfig>> child : acmRoot.getChildren()) {
			root.addChild(fromACMToAMLQueryModel(child));
		}
